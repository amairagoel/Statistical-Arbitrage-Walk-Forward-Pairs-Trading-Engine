...
- **Dynamic OLS Beta ($\beta$):** Instead of static $Spread = Y - X$, we use rolling covariance/variance to calculate the hedge ratio:

$$\beta = \frac{Cov(Y, X)}{Var(X)}$$

Therefore, the spread is defined as $Spread = \ln(Y) - (\beta \times \ln(X))$.
...
- **Walk-Forward Optimization:** Prevents lookahead bias and overfitting by rotating pairs annually (e.g., Train on 2019–2020 → Trade on 2021).
...
| Sharpe Ratio | 1.11 |
| Regime Stability | High (Algorithm successfully rotated out of decaying pairs, such as HDFC/KOTAK in 2022) |
